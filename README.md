...
| Camera motion aggregator | [`camera-motion-aggregator.yaml`](camera-motion-aggregator.yaml) | Aggregate motion/person/vehicle/animal detections from up to 10 cameras over a short window into one push + announcement ("Person, Vehicle at Driveway, Gate"), then pause. |
