...
| Device availability notifier | [`device-availability-notify.yaml`](device-availability-notify.yaml) | Watch up to ~20 devices; at 1-4 daily times send a push report and, on its own 1-4 daily times, a voice announcement of how many are unavailable past a threshold (default 30 min), with optional custom text appended. |
...
| Camera motion aggregator | [`camera-motion-aggregator.yaml`](camera-motion-aggregator.yaml) | Aggregate motion/person/vehicle/animal detections from up to 10 cameras over a short window into one push + announcement ("Person, Vehicle at Driveway, Gate"), then pause; each channel has its own daily time window (default 07:00-22:00). |
| 24h volume schedule | [`volume-schedule-24h.yaml`](volume-schedule-24h.yaml) | Set one media player's volume to a chosen level at up to 10 times of day; between those times the volume is left untouched (manual changes persist). Optional catch-up re-applies the latest slot when the player comes back from off/unavailable. |
